...
var ViewingConditions = class _ViewingConditions {
  static make(whitePoint, adaptingLuminance, backgroundLstar, surround, discountingIlluminant) {
    ...
    return new _ViewingConditions(n2, (2 * rgbA[0] + rgbA[1] + 0.05 * rgbA[2]) * nbb, nbb, ncb, c, nc, rgbD, fl, Math.pow(fl, 0.25), z);
  }
...
var Cam16 = class _Cam16 {
  ...
  static fromJch(j, c, h3) {
    return _Cam16.fromJchInViewingConditions(j, c, h3, ViewingConditions.DEFAULT);
  }
  static fromJchInViewingConditions(j, c, h3, viewingConditions) {
    const q = 4 / viewingConditions.c * Math.sqrt(j / 100) * (viewingConditions.aw + 4) * viewingConditions.fLRoot, m = c * viewingConditions.fLRoot, alpha = c / Math.sqrt(j / 100), s = 50 * Math.sqrt(alpha * viewingConditions.c / (viewingConditions.aw + 4)), hueRadians = h3 * Math.PI / 180, jstar = (1 + 100 * 7e-3) * j / (1 + 7e-3 * j), mstar = 1 / 0.0228 * Math.log(1 + 0.0228 * m), astar = mstar * Math.cos(hueRadians), bstar = mstar * Math.sin(hueRadians);
    return new _Cam16(h3, c, j, q, m, s, jstar, astar, bstar);
  }
  ...
  static fromUcsInViewingConditions(jstar, astar, bstar, viewingConditions) {
    ...
    let h3 = Math.atan2(b, a) * (180 / Math.PI);
    h3 < 0 && (h3 += 360);
    const j = jstar / (1 - 7e-3 * (jstar - 100));
    return _Cam16.fromJchInViewingConditions(j, c, h3, viewingConditions);
  }
  toInt() {
...
var HctSolver = class _HctSolver {
  ...
  static setCoordinate(source, coordinate, target, axis) {
    const t2 = _HctSolver.intercept(source[axis], coordinate, target[axis]);
    return _HctSolver.lerpPoint(source, t2, target);
  }
  static isBounded(x) {
    return 0 <= x && x <= 100;
  }
  static nthVertex(y, n2) {
    const kR = _HctSolver.Y_FROM_LINRGB[0], kG = _HctSolver.Y_FROM_LINRGB[1], kB = _HctSolver.Y_FROM_LINRGB[2], coordA = n2 % 4 <= 1 ? 0 : 100, coordB = n2 % 2 == 0 ? 0 : 100;
    if (n2 < 4) {
...
function colorRgbToHsl(r, g, b) {
  ...
  let h3;
  if (d === 0) h3 = 0;
  else if (max === r) h3 = (g - b) / d % 6;
  else if (max === g) h3 = (b - r) / d + 2;
  else if (max === b) h3 = (r - g) / d + 4;
  const l = (min + max) / 2;
  const s = d === 0 ? 0 : d / (1 - Math.abs(2 * l - 1));
  if (h3 < 0) h3 = 360 / 60 + h3;
  return [h3 * 60, s, l];
}
...
//# sourceMappingURL=chunk-HDXRP3EV.js.map
